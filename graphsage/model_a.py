...
def train(feats, G, all_pairs, tflag, pmos_types, nmos_types):
    ...
    # enc=GNN(feat_dim,hidden_dim,hidden_dim,3,0.5)
    graphsage = SupervisedGraphSage(enc3)
    ...
    for e in range(epoch):
        ...
        for i in range(num_batch):
            ...
            loss=criterion(output,Variable(torch.FloatTensor(np.asarray(sub_label))))
            loss.backward()
            optimizer.step()
            
            pred = output.data.numpy()
            pred = np.where(pred < 0.5, 0, 1)
            train_acc+=f1_score(np.asarray(sub_label), pred)
